...
but the code of `mem_mgr` is much more simplier, and more important, the `mem_mgr` is totally independent from the `aes_pkt` and `uart_pkt`.
{% highlight verilog %}
class test_a extends uvm_test;
...
   mem_mgr m_mem;
...
### Some side notes
Sometimes it seems a bit difficult to follow the flow of data,
we just need to clarify these three places and the action of each one:
